...
require_once APP_PATH .'/services/ErrorCode.php';
require_once APP_PATH .'/services/WXBizDataCrypt.php';
require_once APP_PATH .'/services/Prpcrypt.php';
require_once APP_PATH .'/services/PKCS7Encoder.php';
class WXLoginHelper {

    //默认配置
    protected $config = [
        'url' => "https://api.weixin.qq.com/sns/jscode2session", //微信获取session_key接口url
        'appid' => 'wx68d51a56026931d3', // APPId
        'secret' => '59578d7d3c3bf4592ac6ca8e1e046dc2', // 秘钥
    ...
    public function checkLogin($code, $rawData, $signature, $encryptedData, $iv) {
        ...
        if ($res['code'] !== 200 || !isset($res['result']) || !isset($res['result'])) {
            return ['code'=>ErrorCode::$RequestTokenFailed, 'message'=>'Token失败'];
        }
        ...
        $session3rd = $this->randomFromDev(18);

        $data['session3rd'] = $session3rd;
        $data['sessionKey'] = $sessionKey;
        // $this->load->driver('cache');
        // $this->cache->memcached->save($session3rd, $data['openId'] . $sessionKey);
